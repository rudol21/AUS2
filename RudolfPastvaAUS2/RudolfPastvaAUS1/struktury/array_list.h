...
namespace structures
{
	...
	inline ArrayList<T>::ArrayList() :
		List(),
		array_(new Array<T>(20)),  //incializujeme si velkost pola podla situacie aku potrebujeme
		size_(0)
	{
	}
	...
	{
		delete array_;
		array_ = nullptr; //keby tu posledne dva prikazi neboli tak to staci
		size_ = (0);
	}
	...
	inline void ArrayList<T>::add(const T & data)
	{
		...
		//pouzivam hranate zatvorki na poli  ale pole nieje int
	}
	...
	inline void ArrayList<T>::insert(const T & data, const int index)
	{
		...
		else
		{
			...
			Array<T>::copy(*array_, static_cast<int>(index), *array_, static_cast<int>(index + 1), static_cast<int>(size_ - index));//tu je uz index int takze tam netreba<int> ale pre istotu
			(*array_)[index] = data;
			size_++;
		}
	}
	...
	{
		DSRoutines::rangeCheckExcept(index, size_, "Index index of ArrayList!");
		T prvok = (*array_)[index];
		Array<T>::copy(*array_, index + 1, *array_, index, index - static_cast<int>(size_));
		size_--;
		return prvok;
