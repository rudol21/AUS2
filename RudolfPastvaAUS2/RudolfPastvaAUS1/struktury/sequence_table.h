...
namespace structures
{
	...
	inline T & SequenceTable<K, T>::operator[](const K key)
	{
		...
		if (tableItem != nullptr) {
			...
			throw std::out_of_range("SequenceTable data not found");
		}
	}
	...
	inline bool SequenceTable<K, T>::insert(const K & key, const T & data)
	{
		if (!containsKey(key)) {
			...
			throw std::logic_error("SequenceTble<K,T>::insert: key already present in the table");
		}
	}
	...
	inline T SequenceTable<K, T>::remove(const K & key)
	{
		...
		if (tableItem != nullptr) {
			...
			throw std::logic_error("SequenceTble<K,T>::remove: key not found ");
		}
	}
	// data su vystupny parameter
	template<typename K, typename T>
