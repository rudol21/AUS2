...
namespace structures
{
	...
	inline bool SortedSequenceTable<K, T>::insert(const K & key, const T & data)
	{
		...
		bool found = false;
		//urob tu cyklus while duplicitam sa prida cislo
	...
	{
		int indexOfEnd = static_cast<int>(size());
		if (indexStart == indexOfEnd)
		{
			found = false;
			return indexOfEnd;
		}
		int indexPivota = (indexStart + indexEnd) / 2;
		K klucPivota = (*list_)[indexPivota]->getKey();
		if (klucPivota == key)
		{
			found = true;
			return indexPivota;
		...
		else
		{
			if (indexStart == indexEnd)
			{
				...
				if (key < klucPivota)
					...
					return indexPivota + 1;
			}
			else
			{
				if (klucPivota >= key)
				{
					indexEnd = indexPivota;
				}
				if (klucPivota < key)
				{
					indexStart = indexPivota + 1;
				}
